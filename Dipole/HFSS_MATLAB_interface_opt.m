%% Dipole length optimization
% This script optimizes the length of a dipole to have it resonating at 150 MHz.
%
...
% Antenna Parameters
fC   = 150;          % Frequency of interest [MHz]
Wv   = 3e8/(fC*1e6); % Wavelength [m]
L    = Wv/4;         % Half antenna length [m]
...
for iIters = 1:maxIters
    disp('Starting simulation in HFSS');
    OutputSim = SimHFSS_Matlab(L,PAF);
    Freq = OutputSim{1, 1}{:, 1};
    S11  = OutputSim{1, 1}{:, 2};
    ...
    % Adjust the antenna length in accordance with the discrepancy between the estimated and desired resonance frequency
    L = L*fActual/fC;
...
hold on
plot(Freq, S11)
title("Reflection coefficient for the optimized antenna")
